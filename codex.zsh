...
_codex_session_title() {
    local session_name="$1"
    local host="$(_codex_host)"

    host="${host:-unknown}"
    print -r -- "${host}:${session_name}"
}

_codex_default_title() {
    local host user
...
_codex_tmux_extra_environment_file() {
    print -r -- "${CODEX_TMUX_EXTRA_ENV_FILE:-$HOME/src/codex-tools/codex.tmux.env.local}"
}

_codex_tmux_update_environment() {
    ...
    file="$(_codex_tmux_extra_environment_file)"
    if [[ -r "$file" ]]; then
...
_codex_find_ssh_auth_sock() {
    emulate -L zsh -o extendedglob
    local current="${SSH_AUTH_SOCK:-}"
    local fallback=""
    local pattern="${CODEX_SSH_AUTH_SOCK_GLOB:-/tmp/ssh-*/agent.*}"
    local sock ssh_add_status
    local -a sockets

    if _codex_ssh_auth_sock_valid "$current"; then
        print -r -- "$current"
        return 0
    fi
...
_codex_tmux_sync_environment() {
    local -a tmux_cmd
    local tmux_socket="${CODEX_TMUX_SOCKET:-codex}"
    local tmux_conf="${CODEX_TMUX_CONF:-$HOME/src/codex-tools/codex.tmux.conf}"
    local target="${1:-}"
    local name value
    ...
    for name in COLORTERM DISPLAY SSH_AUTH_SOCK SSH_AGENT_PID SSH_CLIENT SSH_CONNECTION SSH_TTY TERM_PROGRAM TERM_PROGRAM_VERSION XAUTHORITY WAYLAND_DISPLAY; do
        ...
        if [[ "$name" == "SSH_AUTH_SOCK" ]]; then
            value="$(_codex_find_ssh_auth_sock)" || continue
        fi
...
codex_tmux() {
    ...
    tmux_update_environment="$(_codex_tmux_update_environment)"
...
codex_attach() {
    ...
    _codex_tmux_sync_environment "$session"
